...
# List of required packages (tkinter is part of Python's standard library)
...
def dynamic_position_size(current_price, stop_loss_pct, portfolio_value, risk_pct=0.01):
    """
    Determine the number of shares to buy so that risk per trade (current_price * stop_loss_pct)
...
def calculate_trade_recommendation(df, portfolio_value=10000, leverage=5, stop_loss_pct=0.05, take_profit_pct=0.10):
    """
    Based on the latest data, if signal is 1, recommend a BUY trade.
...
def simulate_leveraged_cumulative_return(df, leverage=5):
    ...
    When signal is 1, daily returns are multiplied by the leverage factor.
    """
